...
AMELIE_REFERENCE_REGEX = re.compile(r"^AML:(?P<type>MEM|MDT|ENR|UEN):(?P<id>[0-9]+)(/(?P<ids>[0-9]+(,[0-9]+)*))?$")
...
def retrieve_documents(envelope_id) -> Tuple[List[RetrievedFile], EnvelopeGetResponse]:
    with get_documenso_instance() as doc:
        ...
        if envelope_info.status != EnvelopeGetStatus.COMPLETED:
            raise ValueError(gettext("Requested documents are not completed yet. (EID: {envelope_id})").format(envelope_id=envelope_id))
...
def create_enrollment_documents(person, membership) -> EnvelopeCreateResponse:
    ...
    if isinstance(person, Person):
        ...
        amelie_reference = f"AML:{Person.DOCUMENT_TYPE_ID}:{membership.pk}"
        ...
        amelie_reference = f"AML:{UnverifiedEnrollment.DOCUMENT_TYPE_ID}:{person.pk}"
    else:
    ...
    if len(enrollment_files) > 1:
        ...
        enrollment_message = _(
            ...
            "\n"
            "If you have any questions about this message, please contact the board via e-mail at board@inter-actief.net\n"
            "\n"
            "Kind regards,\n"
    ...
    with translation.override(person.preferred_language):
        return create_document(
            ...
            #  and 'AML:UEN:25' = Amelie UnverifiedEnrollment for Membership #25 and no Authorizations.
...
def create_membership_document(membership) -> EnvelopeCreateResponse:
    ...
    with translation.override(membership.member.preferred_language):
        return create_document(
            ...
            amelie_reference=f"AML:{Membership.DOCUMENT_TYPE_ID}:{membership.pk}",
...
def create_authorization_document(authorization) -> EnvelopeCreateResponse:
    ...
    with translation.override(authorization.person.preferred_language):
        ...
        return create_document(
            ...
            amelie_reference=f"AML:{Authorization.DOCUMENT_TYPE_ID}:{authorization.pk}",
