...
def event_summary(person, event):
    language = person.preferred_language
    if language == "en" and event.summary_en:
        summ = event.summary_en
    else:
        summ = event.summary_nl
    if event.cancelled:
...
def forwards(apps, schema_editor):
    ...
    cash_participations = Participation.objects.filter(payment_method='C').order_by('pk')
    ...
    try:
        cash_payment_method = PaymentMethod.objects.get(name_en='Cash')
        ...
        cash_payment_method = PaymentMethod.objects.create(
            name_en="Cash",
            name_nl="Contant",
            description_en="Cash payment. Money goes into the brown cashbox and the payment is written down on the brown cashbox list with the student number.",
            description_nl="Contante betaling. Geld gaat in de bruine kas en de betaling wordt genoteerd op de bruine kaslijst met het studentnummer.",
            visible=True, visible_activities=True, visible_memberships=True
        )
    ...
    for i, participation in enumerate(cash_participations):
        ...
        with translation.override(participation.person.preferred_language):
            ...
            activity_transaction = ActivityTransaction.objects.create(
                price=price,
                description=_("Manual payment required for enrollment to {event} on {date} (migrated)").format(
                    event=event_summary(participation.person, participation.event),
                    date=participation.event.begin.date(),
                ),
            ...
            if participation.cash_payment_made:
                ...
                create_settlement_for_transactions(
                    ...
                    person=participation.person,
                    description=_("Manual payment for enrollment to {event} on {date} (migrated)").format(
                        event=event_summary(participation.person, participation.event),
                        date=participation.event.begin.date(),
                    ),
