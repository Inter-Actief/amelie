...
class GitlabPlugin(ClaudiaPlugin):
    ...
    def verify_mapping(self, claudia, mp, fix=False):
        ...
        if mp.is_person():
            ...
            if mp.adname:
                # Get committees a user should be in
                committees = [committee for committee in mp.all_groups('ad') if committee.is_committee() and committee.extra_data().get('gitlab', False) and committee.adname and (self.get_group(git, committee.adname) is not None)]
                ...
                if user is None and committees and mp.needs_account():
                ...
                if user is not None:
                    ...
                    def _match(group):
                        return group.path in [g.adname for g in committees]

                    def _remove(group):
                        return group.path not in [g.adname for g in committees]
                    ...
                    add = list(filter(_add, committees))
        ...
        if mp.is_committee() and mp.extra_data().get('gitlab', False):
            logger.debug("Mapping is a committee and should have a GitLab group, checking if the group exists in GitLab.")
