...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('Deleting expired password reset codes...'))
        ...
        for person in people_pass:
            self.stdout.write(self.style.SUCCESS("- Removing password code for {}".format(person.incomplete_name())))
            person.password_reset_code = None
            person.password_reset_expiry = None
            person.save()


        self.stdout.write(self.style.SUCCESS('Deleting expired sudo password reset codes...'))
        ...
        for person in people_sudo:
            self.stdout.write(self.style.SUCCESS("- Removing sudo code for {}".format(person.incomplete_name())))
            person.sudo_reset_code = None
            person.sudo_reset_expiry = None
            person.save()

        self.stdout.write(self.style.SUCCESS('Password reset code cleanup complete!'))
